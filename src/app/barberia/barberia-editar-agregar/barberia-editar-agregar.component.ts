...
/* importamos los servicios "messages" y "carrusel" */
import { MessagesService } from '../../servicios/messages.service';
import { CarruselService } from '../../servicios/carrusel.service';
import { FormControl, FormGroup, Validators } from '@angular/forms';

@Component({
  selector: 'app-barberia-editar-agregar',
  templateUrl: './barberia-editar-agregar.component.html',
  styleUrl: './barberia-editar-agregar.component.css',
})
export class BarberiaEditarAgregarComponent implements OnInit {
  //
  ...
  constructor(
    ...
    this.nombre = this.messagesService.nombreService;
    this.locacion = this.messagesService.locacionService;
    this.servicio = this.messagesService.servicioService;
    this.datosInfo = this.messagesService.datosInfoService;
  ...
  updateInfo(field: string, value: string): void {
    switch (field) {
      case 'nombre':
        this.messagesService.updateNombre(value);
        break;
      case 'locacion':
        this.messagesService.updateLocacion(value);
        break;
      case 'servicio':
        this.messagesService.updateServicio(value);
        break;
      case 'datosInfo':
        this.messagesService.updateDatosInfo(value);
        break;
    }
  ...
  onSubmit() {
    if (this.formServicio.valid) {
      ...
      this.carruselService.saveServiciosCarrusel(this.servicios);
